...
class AppPreference @Inject constructor(
  ...
  private val _isRefreshing =
      MutableStateFlow(sharedPreferences.getBoolean(WIDGET_REFRESHING, false))
  ...
  fun shouldPromptRate(): Boolean = Random.Default.nextInt() % 5 == 0
  ...
  companion object {
    ...
    const val UPDATE_FILTER = "ticker.UPDATE"
    const val SETTING_APP_THEME = "ticker.theme"
    const val SORTED_STOCK_LIST = "SORTED_STOCK_LIST"
    const val PREFS_NAME = "ticker.name"
    const val FONT_SIZE = "ticker.textsize"
    ...
    val DATE_FORMATTER: DateTimeFormatter = DateTimeFormatter.ofLocalizedDate(FormatStyle.MEDIUM)
    val AXIS_DATE_FORMATTER: DateTimeFormatter = DateTimeFormatter.ofPattern("LLL dd-yyyy")

    val DECIMAL_FORMAT: Format = DecimalFormat("#,##0.00##").apply {
      decimalFormatSymbols.apply {
        decimalSeparator = ','
        groupingSeparator = '.'
      }
    }
    val DECIMAL_FORMAT_2DP: Format = DecimalFormat("#,##0.00").apply {
      decimalFormatSymbols.apply {
        decimalSeparator = ','
        groupingSeparator = '.'
      }
