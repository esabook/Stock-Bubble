...
import com.stockbubble.dev.MainActivity
...
import com.stockbubble.dev.hasNotificationPermission
import com.stockbubble.dev.network.data.Quote
import com.stockbubble.dev.network.data.QuoteProperties
import com.stockbubble.dev.provider.AlarmScheduler
...
class NotificationsHandler @Inject constructor(
    ...
    private fun createChannels(): Boolean {
        val hasPermission =
            if (VERSION.SDK_INT >= 33) context.hasNotificationPermission() else true
        if (!hasPermission) {
            return false
        }
        ...
        for (quote in portfolio) {
            when {
                quote.hasAlertAbove() -> {
                    ...
                    with(quote) {
                        if (properties == null) {
                            properties = QuoteProperties(symbol)
                        }
                        properties!!
                    }.also {
                        it.alertAbove = 0.0f
                ...
                quote.changeInPercent.absoluteValue >= 10f -> {
                    if (!hasRecentlyNotifiedGenericAlert(quote)) {
                        ...
                        notificationPrefs.edit {
                            putLong(
                                quote.symbol, ZonedDateTime.now()
                                    .toInstant()
                                    .toEpochMilli()
                            )
                        }
        ...
        if (lastNotifiedTimeMs > 0L) {
            ...
            val lastNotifiedTime =
                ZonedDateTime.ofInstant(
                    Instant.ofEpochMilli(lastNotifiedTimeMs),
...
private class NotificationFactory(
    private val context: Context, private val appPreferences: AppPreference
) {
    ...
    private fun checkPermission(runnable: Runnable){
